...
public class SupervisionTaskService {
    ...
    public SupervisionTaskReassignResponse reassignToOperatorGroup(Long taskId,
        ...
        syncFlowableReassignToGroup(task.flowableTaskId());
    ...
    public SupervisionTaskReassignResponse reassignToUser(Long taskId,
        ...
        syncFlowableReassignToUser(task.flowableTaskId(), targetUser.username());
    ...
    private void syncFlowableReassignToGroup(String flowableTaskId) {
    ...
    private void syncFlowableReassignToUser(String flowableTaskId, String targetUsername) {
        if (flowableTaskId == null || flowableTaskId.isBlank()) {
            return;
        }
        try {
    ...
    private TaskSnapshot loadTaskSnapshot(Long taskId) {
        List<TaskSnapshot> tasks = jdbcTemplate.query(
                "SELECT t.id, t.practice_id, t.flowable_task_id, owner.username AS owner_username "
                        + "FROM task t "
                        + "LEFT JOIN app_user owner ON owner.id = t.owner_user_id "
    ...
    private record TaskSnapshot(Long taskId, Long practiceId, String flowableTaskId, String ownerUsername) {
    }
