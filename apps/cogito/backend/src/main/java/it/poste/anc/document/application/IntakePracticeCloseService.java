...
public class IntakePracticeCloseService {
    ...
    private OwnedTask findOwnedTask(Long practiceId, Long userId) {
        List<OwnedTask> tasks = jdbcTemplate.query(
                "SELECT id, flowable_task_id FROM task WHERE practice_id = ? AND stato = 'IN_CARICO' AND owner_user_id = ?",
                (rs, rowNum) -> new OwnedTask(rs.getLong("id"), rs.getString("flowable_task_id")),
                practiceId,
                userId
        );
    ...
    private void completeFlowableTask(String flowableTaskId, String actorUsername) {
